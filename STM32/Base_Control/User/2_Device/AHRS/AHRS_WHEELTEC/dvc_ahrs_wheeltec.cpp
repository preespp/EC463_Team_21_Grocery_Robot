...
void Class_AHRS_WHEELTEC::Data_Process()
{
    ...
    const uint8_t *raw = reinterpret_cast<const uint8_t *>(UART_Manage_Object->Rx_Buffer);
    const size_t crc16_offset = offsetof(Struct_AHRS_WHEELTEC_UART_Data, CRC_16);
    uint16_t crc16_raw = (uint16_t) raw[crc16_offset] | ((uint16_t) raw[crc16_offset + 1] << 8);
    uint16_t crc16 = (uint16_t) ((crc16_raw >> 8) | (crc16_raw << 8));
    if (Verify_CRC_16((uint8_t *) tmp_buffer + 7, tmp_buffer->Data_Length) != crc16)
    {
        return;
    }
