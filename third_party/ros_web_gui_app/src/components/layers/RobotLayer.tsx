  ...
  private async createCustomLoadingManager(packages?: Record<string, string>): Promise<LoadingManager> {
    ...
    manager.onError = () => {
      if (missingPackages.size > 0) {
        const error = new Error(`Packages missing from configuration: ${Array.from(missingPackages).join(', ')}`);
        console.error('[RobotLayer] LoadingManager error:', error.message);
        throw error;
      }
      if (missingFiles.size > 0) {
        const error = new Error(`Files not found: ${Array.from(missingFiles).join(', ')}`);
        console.error('[RobotLayer] LoadingManager error:', error.message);
        throw error;
      }
    };
    ...
    manager.setURLModifier((url: string) => {
      ...
      if (url.includes('$(find')) {
        ...
        if (fileMatch) {
          ...
          if (!packages || !packages[packageName]) {
            ...
            console.error(`[RobotLayer] URL modifier - Package "${packageName}" is not present in the URDF configuration`);
      ...
      console.error('[RobotLayer] URL modifier - Could not find a matching uploaded asset:', url, 'processed:', processedUrl);
      // 返回一个无效的URL，让LoadingManager触发onError
  ...
  private async loadUrdfModel(): Promise<void> {
    ...
    try {
      ...
      if (!savedConfig.packages || Object.keys(savedConfig.packages).length === 0) {
        const error = new Error('No package references were found in the URDF configuration. Cannot load model.');
