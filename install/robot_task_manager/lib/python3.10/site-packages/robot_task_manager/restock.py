...
def create_restock_tree(ros_node, bt_nodes, bb):

    process_one_item = py_trees.composites.Sequence("ProcessRestockItem", children=[
        ...
        py_trees.composites.Selector("PlaceWithRecovery", children=[
            py_trees.composites.Sequence("PlaceSequence", children=[
                bt_nodes.VerifyPosition(),
                bt_nodes.AdjustOrientation(),
                bt_nodes.MoveGripper(command="open"),
            ]),
            bt_nodes.RetryPlace(max_retries=3),
        ]),
    ...
    root = py_trees.composites.Sequence("RestockRoot", children=[
